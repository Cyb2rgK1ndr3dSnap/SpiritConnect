...
@export var radius := 600 #600
# Interval to generate a random width and height for the room, in tiles.
@export var room_size := Vector2(3, 10) # (2,9) #2 (3, 8) #3 (3, 10)
...
func _ready() -> void:
	position = SpiritConnectUtils.get_rng_point_in_circle(_rng, radius)
...
func _integrate_forces(state: PhysicsDirectBodyState2D) -> void:
	...
	if SpiritConnectUtils.is_approx_equal(_previous_xform.origin, state.transform.origin):
...
func _iter_get(_arg) -> Vector2:
	var width := size.x
	var offset := SpiritConnectUtils.index_to_xy(width, _iter_index)
